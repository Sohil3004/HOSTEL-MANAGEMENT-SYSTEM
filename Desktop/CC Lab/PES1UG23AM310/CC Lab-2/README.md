...
## Troubleshooting
- If templates fail, confirm `templates/` exists and `Jinja2Templates(directory="templates")` points to it.
- For multi-process or production deployments, replace in-memory cache with Redis (recommended).

## Next improvements
- Use Redis for cross-process caching and TTLs.
- Add integration tests and performance scripts that assert latency targets.
- Add metrics (Prometheus) to track cache hit ratio and endpoint latencies.

## Contributing
- Open an issue or pull request; include a short description and any repro steps.

---
Generated by the project assistant — adapt as needed.
